...
import socket, sys, struct, random, array, select, time, ctypes
...
class QueuedPacket:
	...
	def __init__(self, data, reliable=False):
		...
		if self.totalPacketSize() > MAX_PACKET:
			raise Exception("Packet has a size greater than the maximum allowed: %d" % len(self.data))
...
class CoreStack:
	...
	def __handleChunk(self, packet):
		if self.__chunk_list is None:
			self.__chunk_list = []
		self.__chunk_list.append(packet[1:])
	...
	def __processIncomingPacket(self, packet):
		...
		try:
			...
			if type == 0x00:
				...
				if self.__debug:
					print "Handling Core Type: 0x%02X" % type
				...
				if self.__debug:
					print "Handling Game Type: 0x%02X" % type
				event = CoreEvent(EVENT_GAME_PACKET_RECEIVED)
				event.packet = packet
				self.__addPendingEvent(event)
			
		except (IndexError, struct.error):
			print "Error in packet processing"
			print "Packet data: " + event.packet.encode('hex')
